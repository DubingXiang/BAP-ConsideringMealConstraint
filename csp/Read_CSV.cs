...
namespace Dubin_Data
{
    public class CSVReader
    {
        ...
        /// <summary>
        /// 存放的是单个文件的所有列对应的数据，Key为列名，Value为该列的数据
        /// </summary>
        public Dictionary<string, List<string>> ColumnData_Set_;
        /// <summary>
        /// 存放的是每个文件的所有数据
        /// </summary>
        ...
        public CSVReader(string filePath) 
        {
            ...
            ColumnData_Set_ = new Dictionary<string, List<string>>();
        }
        /// <summary>
        /// 以delimete为分隔符读取单个csv文件，
        /// return {column name: column data}
        /// </summary>
        /// <param name="delimete"></param>
        /// <returns></returns>
        public Dictionary<string, List<string>> Read(char[] delimete) 
        {
            ...
            {
                ColumnData_Set_.Add(header[i], value_list[i]);
            }

            return ColumnData_Set_;
        }

        /// <summary>
        /// csv_files：多个csv表的路径集合,形式为文件名，即："filename.csv"
        /// </summary>
        ...
        public Dictionary<string, CSVReader> Read_Multi_CSVs(List<string> csv_files)
        {
            ...
            foreach (var file in csv_files)
            {
                ...
                csv.Read(new char[2] { ',', ' ' });
                s_index=file.LastIndexOf('\\');
                e_index = file.IndexOf('.');
                name=file.Substring(s_index+1,e_index-s_index-1);
                CSV_Set.Add(name, csv);
            }
            return CSV_Set;
        }
