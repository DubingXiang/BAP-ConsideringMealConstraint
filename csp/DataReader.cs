...
namespace CG_CSP_1440
{
    public class DataReader
    {
        ...
        public void Connect_csvs(out List<string> csv_files, string data_dir) 
        {            
            ...
            //传入data下一级目录，用以存放不同案例数据
            csv_files.Add(data_dir + @"\Timetable.csv");
            csv_files.Add(data_dir + @"\CrewBase.csv");
            csv_files.Add(data_dir + @"\CrewRules.csv");
        ...
        {
            CSV_ = CSV_Set_["Timetable"];
            Dictionary<string, List<string>> Timetable_Columns = CSV_.ColumnData_Set_;

            CSVReader CSV_Base = CSV_Set_["CrewBase"];
            Dictionary<string, List<string>> Base_Columns = CSV_Base.ColumnData_Set_;
